...
export default function App() {
  ...
  function HomeScreen({ navigation }) {
    ...
    function showImage(){
      return(
        <Image style={styles.image} source={{ url: item.imageUrl }}/>
      )
    }
            ...
            return (
                <View style={{marginBottom: 50}}>
                    <View style={{flexDirection: 'row'}}>
                      <Image style={styles.profileImage} source={{ url: 'https://firebasestorage.googleapis.com/v0/b/its432-project.appspot.com/o/defaultUser.png?alt=media&token=83c9a0be-7415-41da-b92c-f7322ec741be' }}/>
                      <Text>{item.data.displayname} At {Moment(item.data.createdAt).format("DD MMM YYYY hh:mm:ss")}</Text>
                    </View>
                    <Image style={styles.image} source={{ url: item.data.imageUrl}}/>
